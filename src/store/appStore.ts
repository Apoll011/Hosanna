...
const commitSongLocally = (set: SetFn, get: GetFn, apiSong: ApiSong, previousLocalId?: string) => {
  ...
  const nextSongs = [...songs.filter(s => s.id !== localSong.id && s.id !== previousLocalId), localSong];
  const nextFiles = [...virtualFiles.filter(f => f.path !== localSong.id && f.path !== previousLocalId), { path: localSong.id, content: localSong.content, updatedAt: localSong.updatedAt }];
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setServerUrl: (serverUrl) => { set({ serverUrl }); setStorageItem('cp_server_url', serverUrl); },
  setServerToken: (serverToken) => { set({ serverToken }); setStorageItem('cp_server_token', serverToken); },
  setFontSize: (fontSize) => { set({ fontSize }); setStorageItem('cp_font_size', fontSize); },
  setShowChords: (showChords) => { set({ showChords }); setStorageItem('cp_show_chords', showChords); },
  setShowDiagrams: (showDiagrams) => { set({ showDiagrams }); setStorageItem('cp_show_diagrams', showDiagrams); },
  setKeepScreenAwake: (keepScreenAwake) => { set({ keepScreenAwake }); setStorageItem('cp_keep_awake', keepScreenAwake); },
  setSlowDownOnRepeat: (slowDownOnRepeat) => { set({ slowDownOnRepeat }); setStorageItem('cp_slow_down_repeat', slowDownOnRepeat); },
  setInstrument: (instrument) => { set({ instrument }); setStorageItem('cp_instrument', instrument); },
  setSourceFolderPath: (sourceFolderPath) => { set({ sourceFolderPath }); setStorageItem('cp_source_folder', sourceFolderPath); },
  ...
  getActiveSongListIds: () => {
    ...
    if (context.type === 'service') {
      ...
      return (service.elements || [])
        ...
        .map(e => {
          const localSong = state.songs.find(s => s.remoteId === e.songId) ?? state.songs.find(s => s.id === e.songId);
          return localSong?.id;
        })
  ...
  deleteVirtualFile: async (path) => {
    ...
    if (serverUrl.trim() !== '') {
      const existing = songs.find(s => s.id === path);
      if (existing?.remoteId) await deleteSong(serverUrl, serverToken, existing.remoteId);
    }
  ...
  updateService: async (id, name, date, notes) => {
    ...
    if (serverUrl.trim() !== '') {
      try {
        const updated = await updateServiceApi(serverUrl, serverToken, id, { updatedAt: current.updatedAt || '', name, date, notes });
        commitServiceLocally(set, get, updated);
      } catch (e) { console.error('Failed to update service', e); }
      return;
    }
  ...
  updateServiceElements: async (serviceId, elements) => {
    ...
    if (serverUrl.trim() !== '') {
      try {
        const updated = await updateServiceElementsApi(serverUrl, serverToken, serviceId, { updatedAt: current.updatedAt || '', elements });
        commitServiceLocally(set, get, updated);
      } catch (e) { console.error('Failed to update service elements', e); }
      return;
    }
