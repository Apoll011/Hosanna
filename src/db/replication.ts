...
export function setupReplication(db: HosanaDatabase): ReplicationManager {
            ...
            pull: {
                async handler(lastCheckpoint, batchSize) {
                    try {
                        ...
                        const checkpoint =
                            res.checkpoint ?? lastCheckpoint ?? undefined;
    ...
    const start = () => {
        ...
        const replServices = createCollectionReplication(
            "services",
            db.services,
        );
        ...
        activeReplications.forEach((repl, idx) => {
            ...
            repl.active$.subscribe((isActive) => {
                ...
                if (!navigator.onLine) {
                    ...
                    const anyActive = Array.from(activeStateMap.values()).some(
                        Boolean,
                    );
                    if (!anyActive) {
                        updateStatus("synced");
