 ...
 */
export async function setStorageItemImmediate(key: string, value: unknown): Promise<void> {
  try {
    const db = await getDB();
    ...
    try {
      ...
      console.error(`Fallback write to localStorage failed for ${key}`, e);
    }
  }
}
 ...
 */
export function setStorageItemDebounced(key: string, value: unknown, delay = 300): void {
  ...
  storageTimers[key] = setTimeout(() => {
    setStorageItemImmediate(key, value).catch((err) => {
      console.warn(`Debounced write to IndexedDB failed for key ${key}`, err);
    });
  }, delay);
}
