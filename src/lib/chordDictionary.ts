...
const CHORD_QUALITIES: ChordQuality[] = [
  { id: 'major', label: 'Major', intervals: [0, 4, 7], aliases: ['', 'M', 'maj', 'Maj', 'MAJ'] },
  { id: 'minor', label: 'Minor', intervals: [0, 3, 7], aliases: ['m', 'min', 'Min', 'MIN', '-'] },
  { id: 'dim', label: 'Diminished', intervals: [0, 3, 6], aliases: ['dim', 'o', '\u00B0'] },
  { id: 'aug', label: 'Augmented', intervals: [0, 4, 8], aliases: ['aug', '+'] },
  { id: 'sus2', label: 'Suspended 2nd', intervals: [0, 2, 7], aliases: ['sus2'] },
  { id: 'sus4', label: 'Suspended 4th', intervals: [0, 5, 7], aliases: ['sus4', 'sus'] },
  { id: 'five', label: 'Power chord', intervals: [0, 7], aliases: ['5'] },
  { id: 'six', label: '6th', intervals: [0, 4, 7, 9], aliases: ['6'] },
  { id: 'm6', label: 'Minor 6th', intervals: [0, 3, 7, 9], aliases: ['m6', 'min6'] },
  { id: 'six9', label: '6/9', intervals: [0, 4, 7, 9, 14], aliases: ['6/9', '69'] },
  { id: 'dom7', label: 'Dominant 7th', intervals: [0, 4, 7, 10], aliases: ['7'] },
  { id: 'maj7', label: 'Major 7th', intervals: [0, 4, 7, 11], aliases: ['maj7', 'Maj7', 'MAJ7', 'M7', '\u0394', '\u03947'] },
  { id: 'm7', label: 'Minor 7th', intervals: [0, 3, 7, 10], aliases: ['m7', 'min7', 'Min7'] },
  { id: 'mMaj7', label: 'Minor Major 7th', intervals: [0, 3, 7, 11], aliases: ['mMaj7', 'm(maj7)', 'mM7', 'minMaj7'] },
  { id: 'm7b5', label: 'Half-diminished 7th', intervals: [0, 3, 6, 10], aliases: ['m7b5', 'm7-5', '\u00F8', '\u00F87'] },
  { id: 'dim7', label: 'Diminished 7th', intervals: [0, 3, 6, 9], aliases: ['dim7', 'o7', '\u00B07'] },
  { id: 'aug7', label: '7#5', intervals: [0, 4, 8, 10], aliases: ['7#5', 'aug7'] },
  { id: 'dom7b5', label: '7b5', intervals: [0, 4, 6, 10], aliases: ['7b5'] },
  { id: 'dom7sus4', label: '7sus4', intervals: [0, 5, 7, 10], aliases: ['7sus4'] },
  { id: 'dom7sus2', label: '7sus2', intervals: [0, 2, 7, 10], aliases: ['7sus2'] },
  { id: 'nine', label: '9th', intervals: [0, 4, 7, 10, 14], aliases: ['9'] },
  { id: 'maj9', label: 'Major 9th', intervals: [0, 4, 7, 11, 14], aliases: ['maj9', 'Maj9', 'M9'] },
  { id: 'm9', label: 'Minor 9th', intervals: [0, 3, 7, 10, 14], aliases: ['m9', 'min9'] },
  { id: 'add9', label: 'Add 9', intervals: [0, 4, 7, 14], aliases: ['add9'] },
  { id: 'madd9', label: 'Minor Add 9', intervals: [0, 3, 7, 14], aliases: ['madd9', 'minAdd9'] },
  { id: 'eleven', label: '11th', intervals: [0, 4, 7, 10, 14, 17], aliases: ['11'] },
  { id: 'm11', label: 'Minor 11th', intervals: [0, 3, 7, 10, 14, 17], aliases: ['m11'] },
  { id: 'thirteen', label: '13th', intervals: [0, 4, 7, 10, 14, 17, 21], aliases: ['13'] },
  { id: 'm13', label: 'Minor 13th', intervals: [0, 3, 7, 10, 14, 17, 21], aliases: ['m13'] },
  { id: 'dom7sharp9', label: '7#9', intervals: [0, 4, 7, 10, 15], aliases: ['7#9'] },
  { id: 'dom7flat9', label: '7b9', intervals: [0, 4, 7, 10, 13], aliases: ['7b9'] },
  { id: 'maj7sharp5', label: 'maj7#5', intervals: [0, 4, 8, 11], aliases: ['maj7#5'] },
  { id: 'maj7flat5', label: 'maj7b5', intervals: [0, 4, 6, 11], aliases: ['maj7b5'] },
];
...
const QUALITY_ALIAS_TABLE: Array<{ alias: string; quality: ChordQuality }> = CHORD_QUALITIES
  .flatMap((quality) => quality.aliases.map((alias) => ({ alias, quality })))
...
function resolveQuality(qualitySymbol: string): ChordQuality {
  ...
  const exact = QUALITY_ALIAS_TABLE.find((e) => e.alias !== '' && e.alias === qualitySymbol);
  if (exact) return exact.quality;
  // Loose fallback for typos/odd casing (e.g. "MAJ7", "Sus4").
  const lower = qualitySymbol.toLowerCase();
  const loose = QUALITY_ALIAS_TABLE.find((e) => e.alias !== '' && e.alias.toLowerCase() === lower);
...
const SLASH_CONTAINING_ALIASES = QUALITY_ALIAS_TABLE
  .filter((e) => e.alias.includes('/'))
  .sort((a, b) => b.alias.length - a.alias.length);
...
function parseChordSymbol(chord: string): ParsedChord | null {
  ...
  const slashAlias = SLASH_CONTAINING_ALIASES.find((e) => remainder.startsWith(e.alias));
...
function computePianoVoicing(rootSemitone: number, intervals: number[], bassSemitone?: number) {
...
const E_FORM: Record<string, BarreTemplate> = {
  major: { formRootSemitone: 4, frets: [0, 2, 2, 1, 0, 0], openFingers: [0, 2, 3, 1, 0, 0], barreFingers: [1, 3, 4, 2, 1, 1] },
  minor: { formRootSemitone: 4, frets: [0, 2, 2, 0, 0, 0], openFingers: [0, 2, 3, 0, 0, 0], barreFingers: [1, 3, 4, 1, 1, 1] },
  dom7: { formRootSemitone: 4, frets: [0, 2, 0, 1, 0, 0], openFingers: [0, 2, 0, 1, 0, 0], barreFingers: [1, 3, 1, 2, 1, 1] },
  m7: { formRootSemitone: 4, frets: [0, 2, 0, 0, 0, 0], openFingers: [0, 2, 0, 0, 0, 0], barreFingers: [1, 3, 1, 1, 1, 1] },
  maj7: { formRootSemitone: 4, frets: [0, 2, 1, 1, 0, 0], openFingers: [0, 3, 1, 2, 0, 0], barreFingers: [1, 3, 2, 2, 1, 1] },
  sus4: { formRootSemitone: 4, frets: [0, 2, 2, 2, 0, 0], openFingers: [0, 2, 3, 4, 0, 0], barreFingers: [1, 3, 4, 4, 1, 1] },
};

const A_FORM: Record<string, BarreTemplate> = {
  major: { formRootSemitone: 9, frets: [-1, 0, 2, 2, 2, 0], openFingers: [0, 0, 1, 2, 3, 0], barreFingers: [0, 1, 3, 4, 4, 1] },
  minor: { formRootSemitone: 9, frets: [-1, 0, 2, 2, 1, 0], openFingers: [0, 0, 2, 3, 1, 0], barreFingers: [0, 1, 3, 4, 2, 1] },
  dom7: { formRootSemitone: 9, frets: [-1, 0, 2, 0, 2, 0], openFingers: [0, 0, 1, 0, 2, 0], barreFingers: [0, 1, 3, 1, 4, 1] },
  m7: { formRootSemitone: 9, frets: [-1, 0, 2, 0, 1, 0], openFingers: [0, 0, 2, 0, 1, 0], barreFingers: [0, 1, 3, 1, 2, 1] },
  maj7: { formRootSemitone: 9, frets: [-1, 0, 2, 1, 2, 0], openFingers: [0, 0, 2, 1, 3, 0], barreFingers: [0, 1, 3, 2, 4, 1] },
  sus4: { formRootSemitone: 9, frets: [-1, 0, 2, 2, 3, 0], openFingers: [0, 0, 1, 2, 4, 0], barreFingers: [0, 1, 3, 3, 4, 1] },
  sus2: { formRootSemitone: 9, frets: [-1, 0, 2, 2, 0, 0], openFingers: [0, 0, 1, 2, 0, 0], barreFingers: [0, 1, 3, 4, 1, 1] },
};

function realizeTemplate(t: BarreTemplate, targetSemitone: number): GuitarShape {
...
function templateFingering(qualityId: string, targetSemitone: number): GuitarShape | null {
...
function powerChordShape(targetSemitone: number): GuitarShape {
  ...
  return { frets: [shift, shift + 2, shift + 2, -1, -1, -1], fingers: [1, 3, 4, 0, 0, 0] };
}

const TEMPLATE_QUALITIES = new Set(['major', 'minor', 'dom7', 'm7', 'maj7', 'sus4', 'sus2']);
...
function getGuitarFingering(
  ...
  if (SLASH_CHORD_SHAPES[raw]) return { shape: SLASH_CHORD_SHAPES[raw], approximate: false };
  if (OPEN_CHORD_SHAPES[raw]) return { shape: OPEN_CHORD_SHAPES[raw], approximate: false };
  ...
  if (suffix !== undefined) {
    ...
    if (OPEN_CHORD_SHAPES[canonical]) return { shape: OPEN_CHORD_SHAPES[canonical], approximate: false };
  }

  // 3) Power chords: always exact, any root.
  if (quality.id === 'five') return { shape: powerChordShape(rootSemitone), approximate: false };
  ...
  while (fallbackId && hops < 4) {
    ...
    if (suf !== undefined) {
      ...
      if (OPEN_CHORD_SHAPES[canonical]) return { shape: OPEN_CHORD_SHAPES[canonical], approximate: true };
    }
...
export class DefaultChordDictionary implements IChordDictionary {
  getFingering(chord: string): ChordFingering | null {
    ...
    const piano = computePianoVoicing(parsed.rootSemitone, parsed.quality.intervals, parsed.bassSemitone);
