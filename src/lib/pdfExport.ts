...
export async function exportServiceToPDF(
    ...
    options: {
        ...
        transposeOffsets?: Record<string, number>;
    },
): Promise<void> {
    ...
    const drawPageFooter = () => {
        ...
        doc.text(
            pageText,
            pageWidth - marginX - doc.getTextWidth(pageText),
            pageHeight - 10,
        );
    };
    ...
    songsList.forEach((song, idx) => {
        const customNote =
            (service.elements?.[idx]?.type === "song"
                ? service.elements[idx].notes
                : "") || "";
        unifiedItems.push({
            kind: "song",
            song,
    ...
    doc.text(
        `Total de Itens: ${unifiedItems.length} (${songsList.length} cânticos, ${(service.elements || []).length} elementos)`,
        marginX + 4,
        currentY + 16,
    );
    ...
    if (service.notes) {
        ...
        doc.setTextColor(148, 163, 184);
        doc.text(
            "Nenhuma nota geral adicionada a este culto.",
            marginX + 85,
            currentY + 11,
        );
    }
    ...
    unifiedItems.forEach((item, idx) => {
        ...
        const hasNote =
            item.kind === "song"
                ...
                : item.element?.content &&
                  item.element.content.trim().length > 0;
        ...
        if (item.kind === "song") {
            ...
            doc.setTextColor(71, 85, 105);
            doc.text(
                truncateText(doc, song.folder || "Raiz", 35),
                marginX + 155,
                currentY + 5,
            );
            ...
            if (hasNote) {
                ...
                doc.setTextColor(7, 89, 133);
                doc.text(
                    `Arr.: ${truncateText(doc, item.notes!, 110)}`,
                    marginX + 10,
                    currentY + 11,
                );
            }
            ...
            doc.text(
                translateElement(elem.type as string),
                marginX + 90,
                currentY + 5,
            );
    ...
    if (includeChords) {
        songsList.forEach((song, idx) => {
            ...
            if (songNote.trim().length > 0) {
                ...
                doc.setTextColor(7, 89, 133);
                doc.text(
                    truncateText(doc, songNote, contentWidth - 35),
                    marginX + 30,
                    currentY + 6.2,
                );
