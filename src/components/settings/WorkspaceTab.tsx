...
export const WorkspaceTab: React.FC<WorkspaceTabProps> = ({
  ...
  const { organization, setActiveOrganization, refetch: refetchAuth } = useAuth();
    ...
    <div className="space-y-4">
      ...
      <div className="bg-m3-card dark:bg-m3-dark-card border border-m3-border/40 dark:border-m3-dark-border/40 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
        ...
        {organization?.metadata && typeof organization.metadata === "object" && (
          <div className="p-3 bg-m3-sidebar dark:bg-m3-dark-sidebar rounded-xl border border-m3-border/30 text-xs space-y-1">
            {Boolean((organization.metadata as Record<string, unknown>).description) && (
              <p className="text-m3-text dark:text-m3-dark-text">
                {String((organization.metadata as Record<string, unknown>).description)}
              </p>
            )}
            {Boolean((organization.metadata as Record<string, unknown>).shortName) && (
              <p className="text-[10px] text-m3-secondary dark:text-m3-dark-secondary">
                Nome Curto: <span className="font-bold text-m3-text dark:text-m3-dark-text">{String((organization.metadata as Record<string, unknown>).shortName)}</span>
              </p>
            )}
          </div>
        )}
        ...
        {orgList && orgList.length > 1 && (
          <div className="space-y-2 pt-2 border-t border-m3-border/30">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {orgList.map((org: { id: string; name: string; slug: string }) => {
                ...
                return (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] opacity-75 font-mono truncate">{org.slug}</p>
                    </div>
        ...
        {organization?.members && (
          <div className="flex items-center gap-2 text-xs text-m3-secondary dark:text-m3-dark-secondary">
            ...
            <span>{organization.members.length} membros na organização</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-m3-card dark:bg-m3-dark-card border border-m3-border/40 dark:border-m3-dark-border/40 rounded-2xl p-4 sm:p-5 shadow-xs space-y-3">
        <div>
          ...
          <p className="text-[11px] text-m3-secondary dark:text-m3-dark-secondary mt-0.5">
            URL base onde a API do Hosanna e Better Auth estão alojadas.
          </p>
        </div>
      ...
      <div className="bg-m3-card dark:bg-m3-dark-card border border-m3-border/40 dark:border-m3-dark-border/40 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
        ...
        <div className="bg-m3-sidebar dark:bg-m3-dark-sidebar p-3 rounded-xl border border-m3-border/30 text-xs space-y-1.5 text-m3-secondary dark:text-m3-dark-secondary">
          ...
          <div className="flex justify-between">
            <span className="font-bold text-m3-text dark:text-m3-dark-text">Estado do Sync:</span>
            <span className={`font-bold ${syncStatus === "error" ? "text-red-500" : syncStatus === "syncing" ? "text-m3-primary" : "text-emerald-600 dark:text-emerald-400"}`}>
              {syncStatus === "syncing" ? "A sincronizar..." : syncStatus === "error" ? "Erro na sincronização" : "Sincronizado"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="font-bold text-m3-text dark:text-m3-dark-text">Cânticos Locais:</span>
            <span>{songs.length} cânticos</span>
          </div>
          <div className="flex justify-between">
            <span className="font-bold text-m3-text dark:text-m3-dark-text">Cultos Guardados:</span>
