...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignIn = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        if (
          ...
          (res.error.message?.includes("two") ||
            (res.error as Record<string, unknown>).code === "TWO_FACTOR_REQUIRED")
        ) {
          setIs2FAChallenge(true);
          setErrorMsg("");
        } else {
          setErrorMsg(res.error.message || "Erro ao iniciar sessão. Verifique as credenciais.");
        }
      } else {
        // Success
      ...
      setErrorMsg((err as Error).message || "Falha na ligação com o servidor.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleVerify2FA = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        setErrorMsg(res.error.message || "Código de verificação inválido.");
      } else {
      ...
      setErrorMsg((err as Error).message || "Erro ao verificar código 2FA.");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <form onSubmit={handleVerify2FA} className="space-y-4">
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-m3-secondary dark:text-m3-dark-secondary">
                Esta conta tem a proteção 2FA ativada. Introduza o código de 6 dígitos gerado pelo seu autenticador:
              </p>
            </div>
          ...
          <>
            ...
            <div className="pt-3 border-t border-m3-border/30">
              ...
              >
                ...
                {showServerConfig ? "Ocultar URL do Servidor" : "Configurar URL do Servidor"}
              </button>
